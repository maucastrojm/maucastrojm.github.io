...
<template>
  <div class="flex p-8 w-full">
    ...
    <div class="flex-2 flex pt-12">
      <div
        v-for="(project, index) in content.projects"
        :key="project.name"
        :class="
          cn('w-1/3 mt-6 p-4 border rounded-lg mr-4 shadow-md hover:shadow-lg transition-shadow max-h-16', {
            'cursor-pointer': selectedIndex !== index,
            'border-primary/40': selectedIndex === index,
          })
        "
    ...
    <div class="flex-2 flex pt-12 relative min-h-[400px]">
      <div
